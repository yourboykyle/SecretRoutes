...
public class RouteRecording {
    ...
    public void addWaypoint(WAYPOINT_TYPES type, LocalPlayer player) {
        ...
        if (type == WAYPOINT_TYPES.ENDERPEARLS) {
            ...
            anglePosArray.add(new JsonPrimitive(Minecraft.getInstance().player.getXRot()));
            anglePosArray.add(new JsonPrimitive(RotationUtils.actualToRelativeYaw(Minecraft.getInstance().player.getYRot() % 360, RoomDirectionUtils.roomDirection())));
