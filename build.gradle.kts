...
sourceSets {
    main {
        java {
            srcDir(rootProject.file("src/main/java/common"))
            if (mcData.isLegacyForge) {
                srcDir(rootProject.file("src/main/java/forge-1.8.9"))
            } else if (mcData.isFabric) {
                srcDir(rootProject.file("src/main/java/fabric-1.21.8"))
            }
        }
        resources {
...
dependencies {
    ...
    // LibAutoUpdate - will be shaded into the jar
    // Note: The Deftu shadow plugin automatically shades implementation dependencies
