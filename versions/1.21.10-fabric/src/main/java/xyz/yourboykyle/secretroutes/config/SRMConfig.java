...
public class SRMConfig extends Config {
    ...
    private void getPersonalBestButton() {
        ...
        if (pb != -1) {
            sendChatMessage("Personal best for " + RoomDetection.roomName() + ": " + Formatting.GREEN + PBUtils.formatTime(pb));
        } else {
            sendChatMessage(Formatting.DARK_RED + "No personal best found for " + RoomDetection.roomName());
        }
    }
    ...
    private void setBatWaypointButton() {
        if (Main.routeRecording.recording) {
            ...
            Main.routeRecording.setRecordingMessage(Formatting.YELLOW + "Added bat secret waypoint.");
        } else {
            sendChatMessage(Formatting.RED + "Route recording is not enabled. Press the start recording button to begin.");
        }
    }
    ...
    private void loadColorProfileButton() {
        if (ConfigUtils.loadColorConfig(colorProfileName.isEmpty() ? "default.json" : colorProfileName)) {
            sendChatMessage(Formatting.DARK_GREEN + "Loaded " + Formatting.GREEN + colorProfileName + Formatting.DARK_GREEN + " as color profile");
        }
    }
    ...
    private void listAllColorProfilesButton() {
        sendChatMessage("Color Profiles:", Formatting.DARK_AQUA);
        for (String name : FileUtils.getFileNames(Main.COLOR_PROFILE_PATH)) {
            sendChatMessage(" - " + name, Formatting.AQUA);
        }
    }
